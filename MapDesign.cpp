...
protected:
    std::string assetsPath = "C:/Users/nomie/Desktop/LevelCreator/assets/";
	std::string spriteSheetPath = assetsPath + "32x32.png";
	std::string mapJsonPath = assetsPath + "lobby.json";
	...
	bool OnUserCreate() override {
        ...
        for (int y = 0; y < world.size.y; y++)
			for (int x = 0; x < world.size.x; x++)
			{
				...
				world.GetCell({ x, y }).id[olc::Face::Floor] = olc::vi2d{ 2, 0 } *vTileSize;
...
int main() {
    MapDesigner game;
    if (game.Construct(1024, 512, 2, 2, false))
        game.Start();
    return 0;
}
